  ...
  return (
    <>
      ...
      <div className="warn" style={{ flexDirection: "column", gap: 6 }}>
        <div style={{ fontWeight: 800, color: "#78350f", marginBottom: 2 }}>📲 Como pagar pelo Multicaixa Express</div>
      ...
      >
        ...
        <div className="up-icon">{file ? "📄" : "📤"}</div>
      ...
      {error && (
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 11, padding: "9px 13px", fontSize: 12, color: "#b91c1c", fontWeight: 600, marginBottom: 11 }}>
          ⚠️ {error}
        </div>
      )}

      <div className="info">
        🔒 O sistema valida automaticamente: IBAN de origem, data/hora (máx. 30 min), ID único e valor.
      </div>

      <button className="btn btn-p" onClick={submit} disabled={loading || (!file && !ref.trim())}>
        {loading ? `A enviar... ${prog}%` : "📤 Enviar comprovante"}
      </button>
      <button className="btn btn-o" onClick={onBack}>← Voltar</button>
    </>
  );
}
