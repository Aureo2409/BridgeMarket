...
export function Calculator({ appliedRate, rate, onSubmit, loading, user, kycStep }) {
  ...
  const [field, setField]   = useState("usd"); // which side drives the calc
  ...
  function submit() {
    onSubmit({
      ...
      dest, account, appliedRate: applied,
    });
  }

  const canSubmit = user && usdNum > 0 && account.trim() && !loading;

  return (
    <>
      {/* Hero */}
      <div className="hero">
        <div style={{ fontSize: 10, fontWeight: 700, opacity: .7, textTransform: "uppercase", letterSpacing: .5, marginBottom: 3 }}>
          Taxa de hoje
        </div>
        <div style={{ fontSize: 34, fontWeight: 900, letterSpacing: -2 }}>
          {applied.toLocaleString("pt-AO")} Kz
        </div>
        <div style={{ fontSize: 11, opacity: .7, fontWeight: 600 }}>
          por 1 USD · base {parseFloat(rate.base_rate).toLocaleString("pt-AO")} + margem {parseFloat(rate.margin)} Kz
        </div>
      </div>
      ...
      <div className="card">
        <div className={`calc-box${field === "usd" ? " active" : ""}`} onClick={() => setField("usd")}>
          ...
          <input className="calc-num" type="number" placeholder="0.00"
            value={usd} onChange={e => onUsd(e.target.value)} onFocus={() => setField("usd")} />
          <div className="calc-hint">Digita o valor em dólares</div>
        </div>

        <div className="swap-row">
          <div className="swap-line" />
          <button className="swap-btn" onClick={swap} title="Calcular a partir de Kwanzas">⇅</button>
          <div className="swap-line" />
        </div>

        <div className={`calc-box${field === "aoa" ? " active" : ""}`} onClick={() => setField("aoa")}>
          <div className="calc-flag">🇦🇴 AOA — Kwanza angolano</div>
          <input className="calc-num" type="text" placeholder="0"
            value={aoa} onChange={e => onAoa(e.target.value)} onFocus={() => setField("aoa")} />
      ...
      <div className="card">
        ...
        <div className="dest-grid">
          {DESTS.map(d => (
            <div key={d.id}
              className={`dest-card${dest === d.id ? " sel" : ""}`}
      ...
      {/* Warnings */}
      {!user && (
        <div className="warn">👤 Faz login para criar um pedido.</div>
      )}
      {user && kycStep < 3 && (
        <div className="warn">
          ⚠️ A tua identidade ainda não foi verificada ({kycStep}/3 passos KYC). O pedido ficará em espera.
        </div>
      )}

      <button className="btn btn-p" onClick={submit} disabled={!canSubmit}>
        {loading ? "A criar pedido..." : `Comprar $${usd || "0"} → ${destInfo?.label} ${destInfo?.icon}`}
      </button>
