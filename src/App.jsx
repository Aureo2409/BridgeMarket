...
function AuthScreen() {
  ...
  async function submit() {
    ...
    if (mode === "register") {
      ...
      setErr(error ? error.message : "✅ Verifica o teu email para activar a conta.");
    } else {
    ...
    <div className="shell">
      ...
      <div style={{ position: "relative", zIndex: 2, padding: "44px 22px" }}>
        ...
        <div className="card" style={{ background: "rgba(255,255,255,.96)" }}>
          ...
          <input className="inp" style={{ marginBottom: 14 }} type="password" placeholder="••••••••"
        ...
        <div style={{ display: "flex", gap: 7, justifyContent: "center", flexWrap: "wrap", marginTop: 16 }}>
          {["🔒 KYC Anti-fraude", "💱 Câmbio ao vivo", "📲 MCX Express", "⚡ 4 carteiras"].map(f => (
            <span key={f} style={{ background: "rgba(99,102,241,.09)", border: "1px solid rgba(99,102,241,.2)", borderRadius: 20, padding: "4px 11px", fontSize: 11, fontWeight: 700, color: "#6366f1" }}>{f}</span>
          ))}
        </div>
      </div>
    </div>
...
function ClientApp({ user, onLogout }) {
  const [step, setStep]         = useState(0); // 0=calc 1=confirm 2=proof 3=done
  ...
  useEffect(() => {
    ...
    const ch = sb.channel("client_ch")
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "orders",
        filter: `user_id=eq.${user.id}` }, () => loadOrders())
      .subscribe();
              ...
              <>
                ...
                <div className="card">
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "10px 0 13px" }}>
                    <div style={{ width: 46, height: 46, borderRadius: 13, background: destInfo?.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22 }}>
                      {destInfo?.icon}
                    </div>
                    <div>
                      <div style={{ fontSize: 26, fontWeight: 900, color: "#1e1b4b", letterSpacing: -1 }}>
                        ${parseFloat(currentOrder.amount_usd).toFixed(2)}
                      </div>
                      <div style={{ fontSize: 11, color: "#9ca3af", fontWeight: 600 }}>
                        {parseFloat(currentOrder.amount_aoa).toLocaleString("pt-AO")} Kz · taxa {parseFloat(currentOrder.rate_applied).toLocaleString("pt-AO")} Kz/$
                      </div>
                    </div>
                  </div>
                  {[
                    ["Destino",  `${destInfo?.label} ${destInfo?.icon}`],
                    ["Conta",    currentOrder.destination_account],
            ...
            {step === 3 && (
              <div className="succ">
                ...
                <div className="succ-sub">
                  ...
                  O administrador vai verificar e enviar<br />
                  os dólares para a tua conta <strong>{destInfo?.label}</strong>.
                </div>
